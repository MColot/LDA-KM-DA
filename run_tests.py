...
from testingEnvironment.UDA.VADA_DIRTT import run_VADA_DIRTT
...
if __name__ == "__main__":
    ...
    if "MDD" in testModels:
        ...
        for test in range(len(X)):
            ...
            train = np.delete(train, test)
            if testFeatures == "TDF":
                cl = fit_MDD(np.concatenate(tdfX_n[train]), np.concatenate(Y[train]).astype(int), tdfX_n[test], hidden=50, epochs=20)
            ...
            if testFeatures == "CMTS":
                ...
                tsTrain = np.concatenate([TangentSpace('riemann').fit_transform(x) for x in xTrain])
                tsTest = TangentSpace('riemann').fit_transform(xTest)
                cl = fit_MDD(tsTrain, np.concatenate(Y[train]).astype(int), tsTest, hidden=50, epochs=20)
    ...
    if "VADA and DIRT-T" in testModels:
        multiprocess = False
        if "MDD" in testModels or "MDD -> LDA-KM-DA" in testModels:
            multiprocess = True
        ...
        for test in range(len(X)):
            ...
            if testFeatures == "TDF":
                ...
                predVada = run_VADA_DIRTT(np.concatenate(tdfX_n[train]), np.concatenate(Y[train]).astype(int), tdfX_n[test], Y[test].astype(int), nClasses, saveDirectoryDIRTT, 0, lr, 0.01, 5, 1000, multiprocess)
                tf.compat.v1.reset_default_graph()
                predDirtt = run_VADA_DIRTT(np.concatenate(tdfX_n[train]), np.concatenate(Y[train]).astype(int), tdfX_n[test], Y[test].astype(int), nClasses, saveDirectoryDIRTT, 5000, lr, 0.01, 10, 1000, multiprocess)
                scoreVADA.append(np.mean(Y[test].astype(int) == predVada))
                scoreDIRTT.append(np.mean(Y[test].astype(int) == predDirtt))
            if testFeatures == "CMTS":
                tf.compat.v1.reset_default_graph()
                predVada = run_VADA_DIRTT(np.concatenate(cmtsX[train]), np.concatenate(Y[train]).astype(int), cmtsX[test], Y[test].astype(int), nClasses, saveDirectoryDIRTT, 0, lr, 0.01, 5, 1000, multiprocess)
                tf.compat.v1.reset_default_graph()
                predDirtt = run_VADA_DIRTT(np.concatenate(cmtsX[train]), np.concatenate(Y[train]).astype(int), cmtsX[test], Y[test].astype(int), nClasses, saveDirectoryDIRTT, 5000, lr, 0.01, 10, 1000, multiprocess)
                scoreVADA.append(np.mean(Y[test].astype(int) == predVada))
                scoreDIRTT.append(np.mean(Y[test].astype(int) == predDirtt))
            elif testFeatures == "Xdawn CMTS":
                cov = XdawnCovariances(nfilter=3, estimator='oas').fit(np.concatenate(X[train]), np.concatenate(Y[train]).astype(int))
                ...
                tsTest = TangentSpace('riemann').fit_transform(xTest)
                tf.compat.v1.reset_default_graph()
                predVada = run_VADA_DIRTT(tsTrain, np.concatenate(Y[train]).astype(int), tsTest, Y[test].astype(int), nClasses, saveDirectoryDIRTT, 0, lr, 0.01, 5, 1000, multiprocess)
                tf.compat.v1.reset_default_graph()
                predDirtt = run_VADA_DIRTT(tsTrain, np.concatenate(Y[train]).astype(int), tsTest, Y[test].astype(int), nClasses, saveDirectoryDIRTT, 5000, lr, 0.01, 10, 1000, multiprocess)
                scoreVADA.append(np.mean(Y[test].astype(int) == predVada))
                scoreDIRTT.append(np.mean(Y[test].astype(int) == predDirtt))
    ...
    if "MDD -> LDA-KM-DA" in testModels:
        ...
        for test in range(len(X)):
            ...
            train = np.delete(train, test)
            if testFeatures == "TDF":
                cl = fit_MDD(np.concatenate(tdfX_n[train]), np.concatenate(Y[train]).astype(int), tdfX_n[test], hidden=50, epochs=30)
